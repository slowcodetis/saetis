      ...
      else
      {
         ...
         if($existeFile)
         {
            if(strnatcasecmp($nLargoGE, "Seleccione una grupo empresa") != 0)
            {
                ...
                $fraseClave = 'Notificacion de Conformidad de '. $nCortoGE[0];
                ...
                $cantC1= mysql_num_rows($contrato1);

                $fraseClave = 'Orden de Cambio de '. $nCortoGE[0];
                $consulta1 = "SELECT ID_R FROM `registro` WHERE NOMBRE_R = '$fraseClave' AND  TIPO_T = 'publicaciones'";
                $contrato1 = $conexion->consulta($consulta1);
                ...
                if($cantC1 == 1 || ($cantC2 == 1 && $docRequeridos == $docSubidos)) {
                    ...
                    if($cantC2 == 1 && $docRequeridos > $docSubidos) {
                        echo "<script> alert('La grupo empresa aun no subio los documentos requeridos por la orden de cambio emitida'); window.location='../Vista/contrato.php'; </script>";
                    }
                    else { 
                        echo "<script> alert('Aun no se genero una notificacion de conformidad u orden de cambio para la grupo empresa seleccionada'); window.location='../Vista/contrato.php'; </script>";
                    }
                }
            }
            else {        
