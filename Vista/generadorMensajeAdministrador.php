...
//conexion-------------		
    ...
    if(!empty($_POST['multiselect'])) {
        ...
        foreach ($listaCorreos as $key => $value) {
            $peticion1 = $conectar->consulta("SELECT `CORREO_ELECTRONICO_U` FROM `usuario` WHERE `NOMBRE_U`='$value'"); 
            while ($correo = mysql_fetch_array($peticion1)) {        
                $correo1=$correo["CORREO_ELECTRONICO_U"];
            }
            $listaDestinos[$value] = $correo1;
            //$indice = $indice + 1;
        }
    ...
    //volver a la pagina---------------
        ...
        //$correo1 = "slow.code.srl.tis@gmail.com";
        foreach ($listaDestinos as $destino => $correoDestino) {
            $mail->AddAddress($correoDestino, $destino);
        }
        //$mail->AddAddress($correo1, $addDestino);
        //Definimos el tema del email
        $mail->Subject = 'Mensaje de Administrador SAETIS';
    ...
    //    $mail->MsgHTML('El usuario '.$addDestino.''.$addDestino.' con direccion '.$correo1.' desea registrarse como '.$addDestino.' contraseña: '.$addDestino.'');
        ...
        //Enviamos el correo
        if(!$mail->Send()) {
          echo"<script type=\"text/javascript\">alert('ERROR: mensaje no enviado intente nuevamente $correo1'); window.location='enviarCorreoAdministrador.php';</script>";
        } 
        else {
            //$peticion2 = $conectar->consulta("UPDATE `usuario` SET `ESTADO_E`='Habilitado' WHERE `NOMBRE_U`='$addDestino'");
            echo"<script type=\"text/javascript\">alert('Mensaje enviado exitosamente'); window.location='principal.php';</script>";
        }
    }
    else {
        echo"<script type=\"text/javascript\">alert('Debe seleccionar al menos un destinatario'); window.location='enviarCorreoAdministrador.php';</script>";
    }
?>
